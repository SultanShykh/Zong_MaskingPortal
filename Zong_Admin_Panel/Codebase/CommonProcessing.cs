...
namespace Zong_Admin_Panel.Codebase
{
    public class CommonProcessing
    {
        ...
        public static void GetUserByGroupId(out string selectedUsers, out List<dynamic> users, int? groupId = null)
        {
            ...
            dynamic Records = AppDB.COR_USP_GetUsersByGroupId(groupId: groupId, portalId: 8);
        ...
        public static void GetUser(out List<UserModel> users, int? userid = null)
        {
            ...
            if (Records.FirstOrDefault() != null)
            {
                foreach (var rec in Records)
                {
                    users.Add(new UserModel() { Id = rec.Id, FirstName = rec.FirstName,PortalId = rec.PortalId });
                }
            }
        }
        ...
        public static void GetAllGroups(out List<GroupMasterModel> groups)
        {
            ...
            if (Records.FirstOrDefault() != null)
            {
                foreach (var rec in Records)
                {
                    groups.Add(new GroupMasterModel() { id = rec.Id, Name = rec.Name, Status = rec.Status, isActive = rec.Status == 1 ? "Active" : "Deactive", applicationId = rec.ApplicationId });
                }
            }
        }
        public static void GetGroupWithPermissionsTree(out List<dynamic> groups)
        {
            ...
            dynamic Records = AppDB.COR_USP_GetAllGroupPermissions(portalId: 8);
        ...
        public static void SetPermissions(dynamic Records, string userid, out List<Permissions> groups)
        {
            if (userid != null)
            {
                Records = AppDB.COR_WEB_UserPermissions(Id: userid, portalId: 8);
            }

            groups = new List<Permissions>();

            if (Records.FirstOrDefault() != null)
            {
                foreach (var rec in Records)
                {
                    string MenuParentId = rec.MenuParentId;
                    int FormMasterId = rec.FormMasterId;
                    string FormMasterName = rec.FormMasterName;
                    string MenuName = rec.MenuName;
                    int MenuId = rec.MenuId;
                    
                    string MenuUrl = rec.MenuUrl;
                    string Controller = rec.Controller;
                    string Action = rec.Action;
                    string PermissionId = rec.PermissionId;
                      
                    groups.Add(new Permissions()
                    {
                        FormMasterId = rec.FormMasterId,
                        FormMasterName = rec.FormMasterName,
                        MenuName = rec.MenuName,
        ...
        public static void GetGroupByIdWithPermissionsTree(out List<dynamic> groups, string GroupId = null)
        {
            ...
            dynamic Result = AppDB.COR_USP_GetGroupPermissionsByGroupId(groupId: GroupId, portalId: 8);
